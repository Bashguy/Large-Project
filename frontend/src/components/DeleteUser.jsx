...
function DeleteUser({setConfirmDelete}) {
    ...
    const deleteData = () => {
        console.log(canDelete.password)
        if(canDelete.password === "hallo") {
    ...
    <div className='w-screen h-screen fixed top-0 right-0 flex justify-center items-center'>
      <div className='bg-white p-10 rounded-md shadow-md'>
        ...
        <input 
            ...
            value={canDelete.password}   
            onChange={(e) => {
                setCanDelete({... canDelete, password: e.target.value})
            }}
        />
        <div className='flex justify-between mt-5'>
          ...
          >Cancel</button>
          <button className='outline-1 outline-[#101f20] hover:bg-[#101f20] hover:text-white py-2 px-4 bg-transparent text-black rounded-lg'
           onClick={() => deleteData()}
          >Delete</button>
        </div>
      </div>
